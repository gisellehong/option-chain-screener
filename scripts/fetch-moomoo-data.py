...
def load_active_tracked_contracts(path: Path) -> dict[str, list[dict[str, Any]]]:
    ...
    for trade in payload.get("trades", []):
        ...
        if trade.get("closeDate"):
            continue
        option_type = option_type_value(trade.get("optionType") or ("call" if trade.get("strategy") == "CC" else "put" if trade.get("strategy") in {"CSP", "BPS"} else ""))
        ...
        if trade.get("strategy") == "BPS" and to_float(trade.get("longStrike")) > 0:
            tracked[normalize_code(ticker)].append({"expiration": expiration, "optionType": "put", "strike": to_float(trade["longStrike"])})
    return tracked
