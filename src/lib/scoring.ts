...
export function scoreCandidates(
  ...
  const scored = candidates
    ...
    .sort((left, right) => config.id === "soxl_conservative_csp"
      ? (left.expiryItmProbability ?? 100) - (right.expiryItmProbability ?? 100)
        || (left.touchProbability ?? 100) - (right.touchProbability ?? 100)
        || right.bidAnnualizedRoi - left.bidAnnualizedRoi || right.openInterest - left.openInterest
