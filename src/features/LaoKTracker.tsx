...
type Group = {sourceNote?:string|null;recommendationStatus?:string|null;excludedBuckets?:{displayWeek:number;ticker:string;reason:string}[];date:string;publishedAt:string;snapshotAt:string|null;alignmentMinutes:number|null;decisionMode:string;configVersion:string;configHash:string;sourcePage:string;rows:Row[];extraPicks:Record<string,(Pick & {classification:string})[]>};
const groups=data.groups as unknown as Group[];
const labels:Record<string,string>={outside_universe:'標的範圍不同 · Universe',missing_snapshot:'缺當時快照',missing_contract:'缺合約・原因未知',outside_scope:'未納入抓取 · Scope',not_returned:'已請求未回傳',inventory_not_listed:'合約清單未列出',invalid_quote:'報價不可用',missing_model_data:'缺模型欄位',exact_match:'同合約 · Match',filtered:'門檻淘汰 · Filter',rank_difference:'排序差異 · Ranking',no_pick:'未選出'};
...
export function LaoKTracker(){
 ...
 return <section className="wideWorkspace">
  ...
  <p className="notice">資料收集範圍：SOXL／NBIS／LITE 與老 K 新增標的，未來 63 天 Put、履約價為股價 40%–110%；已追蹤合約持續收集至到期。交易篩選仍使用原有門檻。新範圍不會補回過去缺少的行情。</p>
  <details><summary>如何閱讀比對結果？</summary><p>同合約：雙方選擇一致。排序差異：老 K 合約通過門檻，但我們優先選了另一張。門檻淘汰：至少一項必要條件未通過。缺資料：無法判斷策略是否接受，並非淘汰。新快照區分未納入抓取、已請求未回傳、報價不可用與缺模型欄位；舊快照可能無法追溯原因。</p></details>
  {group.sourceNote && <p className="notice">來源說明：{group.sourceNote}</p>}
  ...
  {group.excludedBuckets?.map(b=><p className="notice" key={`${b.ticker}-${b.displayWeek}`}>老 K 明確留白：第 {b.displayWeek} 週 {b.ticker}。{b.reason}。</p>)}
  ...
  {(group.extraPicks[scenario]?.length ?? 0)>0 && <><h3>Screener 額外選擇</h3><p>{group.extraPicks[scenario].map(p=>`${p.ticker} ${p.expiration} ${p.strike}P${p.classification==='explicit_no_recommendation'?'（老 K 明確不推薦此到期桶）':p.classification==='unlisted_bucket_not_proven_rejected'?'（老 K 未列示此標的／到期桶）':'（不同履約價）'}`).join('；')}。未列示不視為已證實拒絕。</p></>}
