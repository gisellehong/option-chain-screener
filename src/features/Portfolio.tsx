...
export function Portfolio({quotes,quoteAt,legacy}:{quotes:OptionCandidate[];quoteAt:string|null;legacy:ReactNode}){
 ...
 const accounts=Array.from(new Set(['Giselle','Ludi','Patrick GMP',...(ledger?.trades.map(t=>t.account)??[])]));
 ...
 return <section className="wideWorkspace portfolio">
  ...
  {!ledger?<div className="emptyState"><h3>{loading?'讀取本機帳本…':'尚未載入私人帳本'}</h3><p>本機版讀取已同步的 Google Sheet 快照；線上版可匯入交易帳本 JSON，僅儲存在此瀏覽器，不上傳交易資料。</p><p>Patrick GMP 尚待交易來源；不以其他帳戶代替。</p></div>:<>
  ...
  {account==='All'&&!available.some(t=>t.account==='Patrick GMP')&&<p className="notice">目前合計只涵蓋已匯入帳戶；Patrick GMP 尚未納入。</p>}
  ...
  {rows.length===0?<div className="emptyState"><h3>{account==='Patrick GMP'?'Patrick GMP 尚待資料':'此條件沒有交易'}</h3><p>請匯入含對應 account 標籤的私人帳本，或調整篩選條件。</p></div>:<>
